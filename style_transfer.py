...
def main(style_img_path, content_img_path):
    # image 파일명
    style_img_path = style_img_path.split('/')[-1]
    content_img_path = content_img_path.split('/')[-1]
    # image 경로
    style_image_path = os.getcwd()+'/static/images/' + style_img_path  # 스타일 참조 이미지
    content_image_path = os.getcwd()+'/static/images/' + content_img_path  # 타깃 이미지
    ...
    content_image = K.constant(preprocess_image(content_image_path))  # creates img to a constant tensor
    ...
    evaluator = Evaluator()
    style_img_name = style_img_path.split('.')[0].split('/')[-1]
    result_prefix = os.getcwd()+'/static/images/nst_result_' + style_img_name
    iterations = 20
